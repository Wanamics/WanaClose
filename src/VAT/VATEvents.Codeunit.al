codeunit 87220 "wan VAT Events"
{
    ...
    var
        tMustBeCustomerOrVendorForUnrealizedVAT: Label 'must be Customer or Vendor for unrealized VAT';
    begin
        if (VATEntry."Unrealized Base" <> 0) and (VATEntry."Bill-to/Pay-to No." = '') then
            GenJournalLine.FieldError("Source Type", tMustBeCustomerOrVendorForUnrealizedVAT);
    end;
}